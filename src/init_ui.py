...
def validate_input_meta(meta: sly.ProjectMeta):
    ...
    for obj_class in meta.obj_classes:
        if (
            obj_class.geometry_type == sly.Point
            or obj_class.geometry_type == sly.PointLocation
        ):
            classes_with_unsupported_shape.append(
                (obj_class.name, obj_class.geometry_type.geometry_name())
            )

    if len(classes_with_unsupported_shape) > 0:
        sly.logger.warn(
            f"Unsupported shapes: {classes_with_unsupported_shape}. "
...
def prepare_ui_classes(project_meta):
    ...
    for obj_class in project_meta.obj_classes:
        if (
            obj_class.geometry_type == sly.Point
            or obj_class.geometry_type == sly.PointLocation
            or obj_class.geometry_type == sly.Cuboid
        ):
            continue
